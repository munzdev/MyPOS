...
use \Exception;
use \PDO;
use Model\User\UserRoleQuery as ChildUserRoleQuery;
use Model\User\Map\UserRoleTableMap;
...
abstract class UserRole implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\Model\\User\\Map\\UserRoleTableMap';
    ...
    /**
     * Initializes internal state of Model\User\Base\UserRole object.
     */
     ...
     * @return $this|\Model\User\UserRole The current object (for fluent API support)
     */
    public function setUserRoleid($v)
    {
        ...
        if ($this->user_roleid !== $v) {
            $this->user_roleid = $v;
            $this->modifiedColumns[UserRoleTableMap::COL_USER_ROLEID] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : UserRoleTableMap::translateFieldName('UserRoleid', TableMap::TYPE_PHPNAME, $indexType)];
            $this->user_roleid = (null !== $col) ? (int) $col : null;
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\Model\\User\\UserRole'), 0, $e);
        }
    }
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\Model\User\UserRole
     */
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\Model\User\UserRole The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(UserRoleTableMap::COL_USER_ROLEID)) {
            $criteria->add(UserRoleTableMap::COL_USER_ROLEID, $this->user_roleid);
        }
        if ($this->isColumnModified(UserRoleTableMap::COL_NAME)) {
    ...
    {
        $validPk = null !== $this->getUserRoleid();
    ...
    /**
     * Generic method to set the primary key (user_roleid column).
     *
    ...
    {
        return null === $this->getUserRoleid();
    }
     ...
     * objects.
     *
     * @param      object $copyObj An object of \Model\User\UserRole (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Model\User\UserRole Clone of current object.
     * @throws PropelException
     */
