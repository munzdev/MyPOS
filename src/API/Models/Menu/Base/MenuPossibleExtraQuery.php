...
abstract class MenuPossibleExtraQuery extends ModelCriteria
{
     ...
     * </code>
     *
     * @param array[$menu_possible_extraid, $menu_extraid, $menuid] $key Primary key to use for the query
    ...
    public function findPk($key, ConnectionInterface $con = null)
    {
        ...
        if ((null !== ($obj = MenuPossibleExtraTableMap::getInstanceFromPool(serialize([(null === $key[0] || is_scalar($key[0]) || is_callable([$key[0], '__toString']) ? (string) $key[0] : $key[0]), (null === $key[1] || is_scalar($key[1]) || is_callable([$key[1], '__toString']) ? (string) $key[1] : $key[1]), (null === $key[2] || is_scalar($key[2]) || is_callable([$key[2], '__toString']) ? (string) $key[2] : $key[2])]))))) {
    ...
    {
        $sql = 'SELECT menu_possible_extraid, menu_extraid, menuid, price FROM menu_possible_extra WHERE menu_possible_extraid = :p0 AND menu_extraid = :p1 AND menuid = :p2';
        try {
        ...
        if ($row = $stmt->fetch(\PDO::FETCH_NUM)) {
            ...
            $obj->hydrate($row);
            MenuPossibleExtraTableMap::addInstanceToPool($obj, serialize([(null === $key[0] || is_scalar($key[0]) || is_callable([$key[0], '__toString']) ? (string) $key[0] : $key[0]), (null === $key[1] || is_scalar($key[1]) || is_callable([$key[1], '__toString']) ? (string) $key[1] : $key[1]), (null === $key[2] || is_scalar($key[2]) || is_callable([$key[2], '__toString']) ? (string) $key[2] : $key[2])]));
        }
        $stmt->closeCursor();
    ...
    {
        $this->addUsingAlias(MenuPossibleExtraTableMap::COL_MENU_POSSIBLE_EXTRAID, $key[0], Criteria::EQUAL);
        $this->addUsingAlias(MenuPossibleExtraTableMap::COL_MENU_EXTRAID, $key[1], Criteria::EQUAL);
        $this->addUsingAlias(MenuPossibleExtraTableMap::COL_MENUID, $key[2], Criteria::EQUAL);
    ...
    public function filterByPrimaryKeys($keys)
    {
        ...
        foreach ($keys as $key) {
            $cton0 = $this->getNewCriterion(MenuPossibleExtraTableMap::COL_MENU_POSSIBLE_EXTRAID, $key[0], Criteria::EQUAL);
            $cton1 = $this->getNewCriterion(MenuPossibleExtraTableMap::COL_MENU_EXTRAID, $key[1], Criteria::EQUAL);
            $cton0->addAnd($cton1);
            $cton2 = $this->getNewCriterion(MenuPossibleExtraTableMap::COL_MENUID, $key[2], Criteria::EQUAL);
            $cton0->addAnd($cton2);
            $this->addOr($cton0);
        }
    ...
    public function filterByMenuExtra($menuExtra, $comparison = null)
    {
        if ($menuExtra instanceof \API\Models\Menu\MenuExtra) {
            ...
            return $this
                ->addUsingAlias(MenuPossibleExtraTableMap::COL_MENU_EXTRAID, $menuExtra->toKeyValue('MenuExtraid', 'MenuExtraid'), $comparison);
        } else {
    ...
    public function filterByMenu($menu, $comparison = null)
    {
        if ($menu instanceof \API\Models\Menu\Menu) {
            ...
            return $this
                ->addUsingAlias(MenuPossibleExtraTableMap::COL_MENUID, $menu->toKeyValue('Menuid', 'Menuid'), $comparison);
        } else {
    ...
    public function prune($menuPossibleExtra = null)
    {
        if ($menuPossibleExtra) {
            $this->addCond('pruneCond0', $this->getAliasedColName(MenuPossibleExtraTableMap::COL_MENU_POSSIBLE_EXTRAID), $menuPossibleExtra->getMenuPossibleExtraid(), Criteria::NOT_EQUAL);
            $this->addCond('pruneCond1', $this->getAliasedColName(MenuPossibleExtraTableMap::COL_MENU_EXTRAID), $menuPossibleExtra->getMenuExtraid(), Criteria::NOT_EQUAL);
            $this->addCond('pruneCond2', $this->getAliasedColName(MenuPossibleExtraTableMap::COL_MENUID), $menuPossibleExtra->getMenuid(), Criteria::NOT_EQUAL);
