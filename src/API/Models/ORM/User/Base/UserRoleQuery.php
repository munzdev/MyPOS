...
use \Exception;
use \PDO;
use API\Models\ORM\User\UserRole as ChildUserRole;
...
abstract class UserRoleQuery extends ModelCriteria
{
    ...
    /**
     * Initializes internal state of \API\Models\ORM\User\Base\UserRoleQuery object.
     *
     * @param     string $dbName The database name
     * @param     string $modelName The phpName of a model, e.g. 'Book'
     * @param     string $modelAlias The alias for the model in this query, e.g. 'b'
     */
    public function __construct($dbName = 'default', $modelName = '\\API\\Models\\ORM\\User\\UserRole', $modelAlias = null)
    {
    ...
    /**
     * Filter the query on the user_roleid column
     *
     * Example usage:
     * <code>
     * $query->filterByUserRoleid(1234); // WHERE user_roleid = 1234
     * $query->filterByUserRoleid(array(12, 34)); // WHERE user_roleid IN (12, 34)
    ...
    public function filterByUserRoleid($userRoleid = null, $comparison = null)
    {
        if (is_array($userRoleid)) {
            ...
            if (isset($userRoleid['min'])) {
                $this->addUsingAlias(UserRoleTableMap::COL_USER_ROLEID, $userRoleid['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($userRoleid['max'])) {
                $this->addUsingAlias(UserRoleTableMap::COL_USER_ROLEID, $userRoleid['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(UserRoleTableMap::COL_USER_ROLEID, $userRoleid, $comparison);
    }
    ...
    {
        if ($userRole) {
            $this->addUsingAlias(UserRoleTableMap::COL_USER_ROLEID, $userRole->getUserRoleid(), Criteria::NOT_EQUAL);
        }
