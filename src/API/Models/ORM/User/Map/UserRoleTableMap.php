...
namespace API\Models\ORM\User\Map;
...
class UserRoleTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = 'API.Models.ORM.User.Map.UserRoleTableMap';
     ...
     */
    const OM_CLASS = '\\API\\Models\\ORM\\User\\UserRole';

    /**
     * A class that can be returned by this tableMap
     */
    const CLASS_DEFAULT = 'API.Models.ORM.User.UserRole';
    ...
    /**
     * the column name for the user_roleid field
     */
    const COL_USER_ROLEID = 'user_role.user_roleid';
    ...
    protected static $fieldNames = array (
        ...
        self::TYPE_FIELDNAME     => array('user_roleid', 'name', ),
    ...
    protected static $fieldKeys = array (
        ...
        self::TYPE_FIELDNAME     => array('user_roleid' => 0, 'name' => 1, ),
        self::TYPE_NUM           => array(0, 1, )
    );
    ...
    public static function getPrimaryKeyHashFromRow($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        ...
        if ($row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('UserRoleid', TableMap::TYPE_PHPNAME, $indexType)] === null) {
            return null;
        }

        return null === $row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('UserRoleid', TableMap::TYPE_PHPNAME, $indexType)] || is_scalar($row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('UserRoleid', TableMap::TYPE_PHPNAME, $indexType)]) || is_callable([$row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('UserRoleid', TableMap::TYPE_PHPNAME, $indexType)], '__toString']) ? (string) $row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('UserRoleid', TableMap::TYPE_PHPNAME, $indexType)] : $row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('UserRoleid', TableMap::TYPE_PHPNAME, $indexType)];
    }
    ...
    public static function getPrimaryKeyFromRow($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        return (int) $row[
            $indexType == TableMap::TYPE_NUM
                ? 0 + $offset
                : self::translateFieldName('UserRoleid', TableMap::TYPE_PHPNAME, $indexType)
        ];
    }
    ...
    {
        if (null === $alias) {
            $criteria->addSelectColumn(UserRoleTableMap::COL_USER_ROLEID);
            $criteria->addSelectColumn(UserRoleTableMap::COL_NAME);
        } else {
            $criteria->addSelectColumn($alias . '.user_roleid');
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \API\Models\ORM\User\UserRole) { // it's a model object
            // create criteria based on pk values
            $criteria = $values->buildPkeyCriteria();
        } else { // it's a primary key, or an array of pks
            $criteria = new Criteria(UserRoleTableMap::DATABASE_NAME);
            $criteria->add(UserRoleTableMap::COL_USER_ROLEID, (array) $values, Criteria::IN);
        }
