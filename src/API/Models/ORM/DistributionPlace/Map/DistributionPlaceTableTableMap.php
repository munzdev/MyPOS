...
/**
 * This class defines the structure of the 'distribution_place_table' table.
 *
 *
 *
 ...
 *
 */
class DistributionPlaceTableTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = 'API.Models.ORM.DistributionPlace.Map.DistributionPlaceTableTableMap';
     ...
     */
    const TABLE_NAME = 'distribution_place_table';

    /**
     * The related Propel class for this table
     */
    const OM_CLASS = '\\API\\Models\\ORM\\DistributionPlace\\DistributionPlaceTable';

    /**
     * A class that can be returned by this tableMap
     */
    const CLASS_DEFAULT = 'API.Models.ORM.DistributionPlace.DistributionPlaceTable';
    ...
    {
        // attributes
        $this->setName('distribution_place_table');
        $this->setPhpName('DistributionPlaceTable');
        $this->setIdentifierQuoting(true);
        $this->setClassName('\\API\\Models\\ORM\\DistributionPlace\\DistributionPlaceTable');
    ...
    {
        $this->addRelation('DistributionPlaceGroup', '\\API\\Models\\ORM\\DistributionPlace\\DistributionPlaceGroup', RelationMap::MANY_TO_ONE, array (
  0 =>
  array (
    ...
    1 => ':distribution_place_groupid',
        ...
        $this->addRelation('EventTable', '\\API\\Models\\ORM\\Event\\EventTable', RelationMap::MANY_TO_ONE, array (
  0 =>
  array (
    0 => ':event_tableid',
    1 => ':event_tableid',
     ...
     * and findPk*() calls.
     *
     * @param \API\Models\ORM\DistributionPlace\DistributionPlaceTable $obj A \API\Models\ORM\DistributionPlace\DistributionPlaceTable object.
     ...
     *
     * @param mixed $value A \API\Models\ORM\DistributionPlace\DistributionPlaceTable object or a primary key value.
     */
    public static function removeInstanceFromPool($value)
    {
        if (Propel::isInstancePoolingEnabled() && null !== $value) {
            if (is_object($value) && $value instanceof \API\Models\ORM\DistributionPlace\DistributionPlaceTable) {
                ...
                $e = new PropelException("Invalid value passed to removeInstanceFromPool().  Expected primary key or \API\Models\ORM\DistributionPlace\DistributionPlaceTable object; got " . (is_object($value) ? get_class($value) . ' object.' : var_export($value, true)));
                throw $e;
            }
    ...
    {
        return $withPrefix ? DistributionPlaceTableTableMap::CLASS_DEFAULT : DistributionPlaceTableTableMap::OM_CLASS;
    }
     ...
     * @return array           (DistributionPlaceTable object, last column rank)
     */
    public static function populateObject($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        $key = DistributionPlaceTableTableMap::getPrimaryKeyHashFromRow($row, $offset, $indexType);
        if (null !== ($obj = DistributionPlaceTableTableMap::getInstanceFromPool($key))) {
            // We no longer rehydrate the object, since this can cause data loss.
            // See http://www.propelorm.org/ticket/509
            // $obj->hydrate($row, $offset, true); // rehydrate
            $col = $offset + DistributionPlaceTableTableMap::NUM_HYDRATE_COLUMNS;
        } else {
            $cls = DistributionPlaceTableTableMap::OM_CLASS;
            /** @var DistributionPlaceTable $obj */
            $obj = new $cls();
            $col = $obj->hydrate($row, $offset, false, $indexType);
            DistributionPlaceTableTableMap::addInstanceToPool($obj, $key);
        }
    ...
    public static function populateObjects(DataFetcherInterface $dataFetcher)
    {
        ...
        while ($row = $dataFetcher->fetch()) {
            $key = DistributionPlaceTableTableMap::getPrimaryKeyHashFromRow($row, 0, $dataFetcher->getIndexType());
            if (null !== ($obj = DistributionPlaceTableTableMap::getInstanceFromPool($key))) {
                // We no longer rehydrate the object, since this can cause data loss.
                // See http://www.propelorm.org/ticket/509
    ...
    {
        return Propel::getServiceContainer()->getDatabaseMap(DistributionPlaceTableTableMap::DATABASE_NAME)->getTable(DistributionPlaceTableTableMap::TABLE_NAME);
    }
    ...
    {
        $dbMap = Propel::getServiceContainer()->getDatabaseMap(DistributionPlaceTableTableMap::DATABASE_NAME);
        if (!$dbMap->hasTable(DistributionPlaceTableTableMap::TABLE_NAME)) {
            $dbMap->addTableObject(new DistributionPlaceTableTableMap());
        }
    }

    /**
     * Performs a DELETE on the database, given a DistributionPlaceTable or Criteria object OR a primary key value.
     *
     * @param mixed               $values Criteria or DistributionPlaceTable object or primary key or array of primary keys
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \API\Models\ORM\DistributionPlace\DistributionPlaceTable) { // it's a model object
        ...
        $query = DistributionPlaceTableQuery::create()->mergeWith($criteria);

        if ($values instanceof Criteria) {
            DistributionPlaceTableTableMap::clearInstancePool();
        } elseif (!is_object($values)) { // it's a primary key, or an array of pks
            foreach ((array) $values as $singleval) {
                DistributionPlaceTableTableMap::removeInstanceFromPool($singleval);
            }
        }
    ...
    {
        return DistributionPlaceTableQuery::create()->doDeleteAll($con);
    }

    /**
     * Performs an INSERT on the database, given a DistributionPlaceTable or Criteria object.
     *
     * @param mixed               $criteria Criteria or DistributionPlaceTable object containing data that is used to create the INSERT statement.
    ...
    public static function doInsert($criteria, ConnectionInterface $con = null)
    {
        ...
        if ($criteria instanceof Criteria) {
            ...
            $criteria = $criteria->buildCriteria(); // build Criteria from DistributionPlaceTable object
        }
...
} // DistributionPlaceTableTableMap
// This is the static code needed to register the TableMap for this table with the main Propel class.
//
DistributionPlaceTableTableMap::buildTableMap();
