...
abstract class EventTableQuery extends ModelCriteria
{
    ...
    {
        if ($criteria instanceof ChildEventTableQuery) {
            return $criteria;
        }
        $query = new ChildEventTableQuery();
        if (null !== $modelAlias) {
     ...
     * </code>
     *
     * @param array[$event_tableid, $eventid] $key Primary key to use for the query
    ...
    public function findPk($key, ConnectionInterface $con = null)
    {
        ...
        if ((null !== ($obj = EventTableTableMap::getInstanceFromPool(serialize([(null === $key[0] || is_scalar($key[0]) || is_callable([$key[0], '__toString']) ? (string) $key[0] : $key[0]), (null === $key[1] || is_scalar($key[1]) || is_callable([$key[1], '__toString']) ? (string) $key[1] : $key[1])]))))) {
     ...
     *
     * @return ChildEventTable A model object, or null if the key is not found
     */
    protected function findPkSimple($key, ConnectionInterface $con)
    {
        $sql = 'SELECT event_tableid, eventid, name, data FROM event_table WHERE event_tableid = :p0 AND eventid = :p1';
        try {
        ...
        if ($row = $stmt->fetch(\PDO::FETCH_NUM)) {
            ...
            $obj->hydrate($row);
            EventTableTableMap::addInstanceToPool($obj, serialize([(null === $key[0] || is_scalar($key[0]) || is_callable([$key[0], '__toString']) ? (string) $key[0] : $key[0]), (null === $key[1] || is_scalar($key[1]) || is_callable([$key[1], '__toString']) ? (string) $key[1] : $key[1])]));
        }
        $stmt->closeCursor();
    ...
    {
        $this->addUsingAlias(EventTableTableMap::COL_EVENT_TABLEID, $key[0], Criteria::EQUAL);
        $this->addUsingAlias(EventTableTableMap::COL_EVENTID, $key[1], Criteria::EQUAL);
    ...
    public function filterByPrimaryKeys($keys)
    {
        ...
        foreach ($keys as $key) {
            $cton0 = $this->getNewCriterion(EventTableTableMap::COL_EVENT_TABLEID, $key[0], Criteria::EQUAL);
            $cton1 = $this->getNewCriterion(EventTableTableMap::COL_EVENTID, $key[1], Criteria::EQUAL);
            $cton0->addAnd($cton1);
            $this->addOr($cton0);
        }
    ...
    public function filterByEventTableid($eventTableid = null, $comparison = null)
    {
        if (is_array($eventTableid)) {
            ...
            if (isset($eventTableid['min'])) {
                $this->addUsingAlias(EventTableTableMap::COL_EVENT_TABLEID, $eventTableid['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($eventTableid['max'])) {
                $this->addUsingAlias(EventTableTableMap::COL_EVENT_TABLEID, $eventTableid['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(EventTableTableMap::COL_EVENT_TABLEID, $eventTableid, $comparison);
    }
    ...
    public function filterByEventid($eventid = null, $comparison = null)
    {
        if (is_array($eventid)) {
            ...
            if (isset($eventid['min'])) {
                $this->addUsingAlias(EventTableTableMap::COL_EVENTID, $eventid['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($eventid['max'])) {
                $this->addUsingAlias(EventTableTableMap::COL_EVENTID, $eventid['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(EventTableTableMap::COL_EVENTID, $eventid, $comparison);
    }
    ...
    public function filterByName($name = null, $comparison = null)
    {
        ...
        return $this->addUsingAlias(EventTableTableMap::COL_NAME, $name, $comparison);
    }
    ...
    public function filterByEvent($event, $comparison = null)
    {
        if ($event instanceof \API\Models\Event\Event) {
            return $this
                ->addUsingAlias(EventTableTableMap::COL_EVENTID, $event->getEventid(), $comparison);
            ...
            return $this
                ->addUsingAlias(EventTableTableMap::COL_EVENTID, $event->toKeyValue('PrimaryKey', 'Eventid'), $comparison);
        } else {
    ...
    public function filterByDistributionPlaceTable($distributionPlaceTable, $comparison = null)
    {
        if ($distributionPlaceTable instanceof \API\Models\DistributionPlace\DistributionPlaceTable) {
            return $this
                ->addUsingAlias(EventTableTableMap::COL_EVENT_TABLEID, $distributionPlaceTable->getEventTableid(), $comparison);
    ...
    public function filterByOrder($order, $comparison = null)
    {
        if ($order instanceof \API\Models\Ordering\Order) {
            return $this
                ->addUsingAlias(EventTableTableMap::COL_EVENT_TABLEID, $order->getEventTableid(), $comparison);
     ...
     * @return $this|ChildEventTableQuery The current query, for fluid interface
     */
    public function prune($eventTable = null)
    {
        if ($eventTable) {
            $this->addCond('pruneCond0', $this->getAliasedColName(EventTableTableMap::COL_EVENT_TABLEID), $eventTable->getEventTableid(), Criteria::NOT_EQUAL);
            $this->addCond('pruneCond1', $this->getAliasedColName(EventTableTableMap::COL_EVENTID), $eventTable->getEventid(), Criteria::NOT_EQUAL);
    ...
    public function delete(ConnectionInterface $con = null)
    {
        ...
        $criteria->setDbName(EventTableTableMap::DATABASE_NAME);
