...
class OrdersTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = 'API.Models.Ordering.Map.OrdersTableMap';
     ...
     */
    const OM_CLASS = '\\API\\Models\\Ordering\\Orders';

    /**
     * A class that can be returned by this tableMap
     */
    const CLASS_DEFAULT = 'API.Models.Ordering.Orders';
     ...
     */
    const COL_ORDERID = 'orders.orderid';

    /**
     * the column name for the eventid field
     */
    const COL_EVENTID = 'orders.eventid';

    /**
     * the column name for the tableid field
     */
    const COL_TABLEID = 'orders.tableid';

    /**
     * the column name for the userid field
     */
    const COL_USERID = 'orders.userid';

    /**
     * the column name for the ordertime field
     */
    const COL_ORDERTIME = 'orders.ordertime';

    /**
     * the column name for the priority field
     */
    const COL_PRIORITY = 'orders.priority';

    /**
     * the column name for the finished field
     */
    const COL_FINISHED = 'orders.finished';
    ...
    protected static $fieldNames = array (
        self::TYPE_PHPNAME       => array('Orderid', 'Eventid', 'Tableid', 'Userid', 'Ordertime', 'Priority', 'Finished', ),
        self::TYPE_CAMELNAME     => array('orderid', 'eventid', 'tableid', 'userid', 'ordertime', 'priority', 'finished', ),
        self::TYPE_COLNAME       => array(OrdersTableMap::COL_ORDERID, OrdersTableMap::COL_EVENTID, OrdersTableMap::COL_TABLEID, OrdersTableMap::COL_USERID, OrdersTableMap::COL_ORDERTIME, OrdersTableMap::COL_PRIORITY, OrdersTableMap::COL_FINISHED, ),
        self::TYPE_FIELDNAME     => array('orderid', 'eventid', 'tableid', 'userid', 'ordertime', 'priority', 'finished', ),
    ...
    protected static $fieldKeys = array (
        self::TYPE_PHPNAME       => array('Orderid' => 0, 'Eventid' => 1, 'Tableid' => 2, 'Userid' => 3, 'Ordertime' => 4, 'Priority' => 5, 'Finished' => 6, ),
        self::TYPE_CAMELNAME     => array('orderid' => 0, 'eventid' => 1, 'tableid' => 2, 'userid' => 3, 'ordertime' => 4, 'priority' => 5, 'finished' => 6, ),
        self::TYPE_COLNAME       => array(OrdersTableMap::COL_ORDERID => 0, OrdersTableMap::COL_EVENTID => 1, OrdersTableMap::COL_TABLEID => 2, OrdersTableMap::COL_USERID => 3, OrdersTableMap::COL_ORDERTIME => 4, OrdersTableMap::COL_PRIORITY => 5, OrdersTableMap::COL_FINISHED => 6, ),
        self::TYPE_FIELDNAME     => array('orderid' => 0, 'eventid' => 1, 'tableid' => 2, 'userid' => 3, 'ordertime' => 4, 'priority' => 5, 'finished' => 6, ),
        self::TYPE_NUM           => array(0, 1, 2, 3, 4, 5, 6, )
    );
    ...
    public function initialize()
    {
        ...
        $this->setClassName('\\API\\Models\\Ordering\\Orders');
  ...
  array (
    ...
    1 => ':orderid',
        ...
        $this->addRelation('OrdersInProgress', '\\API\\Models\\OIP\\OrdersInProgress', RelationMap::ONE_TO_MANY, array (
  0 =>
  array (
    0 => ':orderid',
    1 => ':orderid',
     ...
     * and findPk*() calls.
     *
     * @param \API\Models\Ordering\Orders $obj A \API\Models\Ordering\Orders object.
    ...
    public static function addInstanceToPool($obj, $key = null)
    {
        if (Propel::isInstancePoolingEnabled()) {
            if (null === $key) {
                $key = serialize([(null === $obj->getOrderid() || is_scalar($obj->getOrderid()) || is_callable([$obj->getOrderid(), '__toString']) ? (string) $obj->getOrderid() : $obj->getOrderid()), (null === $obj->getEventid() || is_scalar($obj->getEventid()) || is_callable([$obj->getEventid(), '__toString']) ? (string) $obj->getEventid() : $obj->getEventid()), (null === $obj->getTableid() || is_scalar($obj->getTableid()) || is_callable([$obj->getTableid(), '__toString']) ? (string) $obj->getTableid() : $obj->getTableid()), (null === $obj->getUserid() || is_scalar($obj->getUserid()) || is_callable([$obj->getUserid(), '__toString']) ? (string) $obj->getUserid() : $obj->getUserid())]);
            } // if key === null
     ...
     *
     * @param mixed $value A \API\Models\Ordering\Orders object or a primary key value.
     */
    public static function removeInstanceFromPool($value)
    {
        if (Propel::isInstancePoolingEnabled() && null !== $value) {
            if (is_object($value) && $value instanceof \API\Models\Ordering\Orders) {
                $key = serialize([(null === $value->getOrderid() || is_scalar($value->getOrderid()) || is_callable([$value->getOrderid(), '__toString']) ? (string) $value->getOrderid() : $value->getOrderid()), (null === $value->getEventid() || is_scalar($value->getEventid()) || is_callable([$value->getEventid(), '__toString']) ? (string) $value->getEventid() : $value->getEventid()), (null === $value->getTableid() || is_scalar($value->getTableid()) || is_callable([$value->getTableid(), '__toString']) ? (string) $value->getTableid() : $value->getTableid()), (null === $value->getUserid() || is_scalar($value->getUserid()) || is_callable([$value->getUserid(), '__toString']) ? (string) $value->getUserid() : $value->getUserid())]);
                ...
                $e = new PropelException("Invalid value passed to removeInstanceFromPool().  Expected primary key or \API\Models\Ordering\Orders object; got " . (is_object($value) ? get_class($value) . ' object.' : var_export($value, true)));
                throw $e;
            }
    ...
    {
        if (null === $alias) {
            $criteria->addSelectColumn(OrdersTableMap::COL_ORDERID);
            $criteria->addSelectColumn(OrdersTableMap::COL_EVENTID);
            $criteria->addSelectColumn(OrdersTableMap::COL_TABLEID);
            $criteria->addSelectColumn(OrdersTableMap::COL_USERID);
            $criteria->addSelectColumn(OrdersTableMap::COL_ORDERTIME);
            $criteria->addSelectColumn(OrdersTableMap::COL_PRIORITY);
            $criteria->addSelectColumn(OrdersTableMap::COL_FINISHED);
        } else {
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \API\Models\Ordering\Orders) { // it's a model object
            ...
            foreach ($values as $value) {
                $criterion = $criteria->getNewCriterion(OrdersTableMap::COL_ORDERID, $value[0]);
                $criterion->addAnd($criteria->getNewCriterion(OrdersTableMap::COL_EVENTID, $value[1]));
                $criterion->addAnd($criteria->getNewCriterion(OrdersTableMap::COL_TABLEID, $value[2]));
                $criterion->addAnd($criteria->getNewCriterion(OrdersTableMap::COL_USERID, $value[3]));
    ...
    public static function doInsert($criteria, ConnectionInterface $con = null)
    {
        ...
        if ($criteria->containsKey(OrdersTableMap::COL_ORDERID) && $criteria->keyContainsValue(OrdersTableMap::COL_ORDERID) ) {
