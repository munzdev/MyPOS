...
abstract class StatusCheck
{
    ...
    public static function verifyInvoice(int $i_invoiceid) : void {
        $o_invoice = InvoiceQuery::create()
                                    ...
                                    ->endUse()
                                    ->joinInvoiceItem()
                                    ->withColumn("SUM(" . InvoiceItemTableMap::COL_AMOUNT . " * " . InvoiceItemTableMap::COL_PRICE . ")", "InvoiceAmount")
                                    ->withColumn("IFNULL(SUM(" . PaymentTableMap::COL_AMOUNT_RECIEVED . "), 0)", "PaymentAmount")
                                    ->groupByInvoiceid()
        ...
        if($o_invoice->getVirtualColumn('InvoiceAmount') == $o_invoice->getVirtualColumn('PaymentAmount') && $d_payment_finished == null) 
            $d_payment_finished = new DateTime();
        elseif($o_invoice->getVirtualColumn('InvoiceAmount') != $o_invoice->getVirtualColumn('PaymentAmount') && $d_payment_finished != null) 
